...
nightmare
  ...
  .select('select#map-selector', 'Legionellosis')
  ...
  .evaluate(() => {
    ...
    const legionellosisData = case_counts.map(count => {
      return {
        state_id: stateId++,
        case_count: count,
        disease_id: 4
    ...
    return legionellosisData;
  })
  .end()
  .then(result => {
    let data = JSON.stringify(result, null, 2);

    fs.writeFile('./legionellosis-data.json', data, 'utf8', err => {
      if (err) {
